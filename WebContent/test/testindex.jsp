...
// 		  alert($("#prizeDiv").html());
		  ...
		  var html = temp.replace(new RegExp("prizetype0","gm"),'prizetype'+addIndex).replace(new RegExp("prizeinfo0","gm"),'prizeinfo'+addIndex);
// 		  $(temp).html(html);
		  // 		  $(temp).html().replace(new RegExp(/(prizetype0)/g),'prizetype'+addIndex).replace(new RegExp(/(prizeinfo0)/g),'prizeinfo'+addIndex)
//		  $('').appendTo($("#prizeDiv"));
// 		  alert($(temp).html());
// 		  alert($("#prizeDiv").html());
		  $("#prizeDiv").append('<br>'+html);
...
// 		  $("#prizeDiv").append('<span id="deletePrizeDiv"><a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="removePrize('+addIndex+')">删除获奖信息</a></span>');
		  $("#prizeDiv").append('<a id="deletePrizeBtn" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="removePrize('+addIndex+')">删除获奖信息</a>');
		  addIndex++;
	}
</script>
</body>
