...
public class ShopCartAction extends BaseAction {
	...
	public Item isExistsItem(Item item) {
		...
		if (null == list || list.size() == 0) {
			...
			for (Item row : list) {
				if (row.getBsProduct().getId()
						.equals(item.getBsProduct().getId())) {
					returnitem = row;
				}
			}
		}
	...
	public String pay() {
		...
		try {
			orderVo = localServiceProxy.Pay(bsAddress, (Shopcart) getSession()
					.get(Constant.SHOP_CART), identity);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return "pay";
